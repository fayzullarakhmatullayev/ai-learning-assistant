...
if (!process.env.GEMINI_API_KEY) {
  console.error('GEMINI_API_KEY is not set. Please set it in the .env file.');
  process.exit(1);
}
 ...
 * @param {string} prompt - Document text
...
export const generateFlashcards = async (text, count = 10) => {
  ...
  try {
    const generatedText = await aiGenerateText(prompt);
