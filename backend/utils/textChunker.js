/**
 * @description Split Text info chunks for better AI processing
...
export const chunkText = (text, chunkSize = 500, overlap = 50) => {
  ...
  for (const paragraph of paragraphs) {
    ...
    // If single paragrap exceeds chunk size, split it by words
...
/**
 * @description Find relevant chunks based on keywords matching
...
export const findRelevantChunks = (chunks, query, maxChunks = 3) => {
  ...
  const scoredChunks = chunks.map((chunk, index) => {
    ...
    // Normilize by content length
    const normilizedScore = score / Math.sqrt(contentWords);
    ...
    return {
      ...
      pageNumber: chunk.pageNumber,
      _id: chunk._id,
      score: normilizedScore * positionBonus,
      rawScore: score,
